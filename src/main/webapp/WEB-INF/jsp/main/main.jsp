...
<script type="text/javascript">
				...
				alert("확장자가 안 맞으면 멈추기");
                                 ...
                                 sdLayer = new ol.layer.Tile(
                                       { // sd 시도
                                          source : new ol.source.TileWMS(
                                                {
                                                   url : 'http://wisejia.iptime.org:8080/geoserver/cite/wms?service=WMS', // 1. 레이어 URL
                                                   params : {
                                                      'VERSION' : '1.1.0', // 2. 버전
				                   ...
				                   type: 'POST', // 또는 "GET", 요청 방식 선택
				                     ...
				                     success: function(response) {
				                         ...
				                         // for(var i = 0; i < sgg.length; i++) {
				                        	 ...
				                        	 /*  alert("for문으로 진입"); */
				                              //var item = sgg[i];
				                              console.log("for문 안으로 진입" + item.sgg_nm);                          
				 ...
				 let s_cd;
                  $('#sggSelect').change(function() {
                	  ...
                	  //console.log("sggSelectedValue" + sggSelectedValue);
                              ...
                              //	alert(sggSelectedValue);
                                 ...
                                 $.ajax({
                                          ...
                                          dataType : 'json',
                                          success : function(response) { //controller에서 return map이 response(변수명)랑 같은 것
                                             ...
                                             //for (var i = 0; i < bjd.length; i++) {
                                               ...
                                               // var item = bjd[i];
                                                 console.log("for문 안으로 진입"+item.bjd_nm); 
                	    ...
                	    $.ajax({
                	        ...
                	        success: function(result) {
                	            ...
                	            var bjdSource = new ol.source.TileWMS({
                	                url: 'http://wisejia.iptime.org:8080/geoserver/cite/wms?service=WMS',
                	                params: {
                	                    'VERSION': '1.1.0',
                	                    'LAYERS': 'cite:e6bjdview',
                	                    'CQL_FILTER': bjd_CQL,
                  ...
                  map.on('singleclick', function(evt) {
                    ...
                    if (sggBjdFlag == 'sgg') {
                        ...
                        var featureRequest = new ol.format.WFS().writeGetFeature({
                          srsName: 'EPSG:3857',
                          featureNS: 'http://wisejia.iptime.org:8080/geoserver/cite',
                          featurePrefix: 'cite',
                          featureTypes: ['e6bjdview'],
                    ...
                    // 서버에 요청 보내기
                    fetch('http://wisejia.iptime.org:8080/geoserver/cite/wfs', {
                      method: 'POST',
  ...
  // GeoServer의 REST API 엔드포인트 설정
 	const baseUrl = 'http://wisejia.iptime.org:8080/geoserver';
  ...
  $.ajax({
      ...
      success: function(response) {
          ...
          // 여기서 범례 이미지 URL을 사용하여 이미지를 화면에 표시하거나 추가적인 처리를 할 수 있다
      },
      error: function(xhr, status, error) {
...
function updateLegend(breakValues) {
  ...
  breakValues.forEach((value, index) => {
      ...
      if(style == 'totalusagedgg'){
    	  colorSpan.style.backgroundColor = getColorDgg(); // getColor 함수는 각 값에 해당하는 색상을 반환하는 것으로 가정한다.
      }else if(style == 'totalusagenature'){
    	  colorSpan.style.backgroundColor = getColorNature(); // getColor 함수는 각 값에 해당하는 색상을 반환하는 것으로 가정한다.
      }
...
<style type="text/css">
  ...
  /* 폰트 스타일 */
  body {
    font-family: 'Arial', sans-serif; /* 원하는 폰트 이름 넣기 */
  }
